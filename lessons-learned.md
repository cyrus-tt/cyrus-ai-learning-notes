...
## L008 · 冒烟测试指向一个从不存在的域名 = 永远红灯，掩盖一切真问题（2026-06-12）

**现象**：deploy workflow 连续 3+ 次失败，排查发现部署步骤一直成功，失败的全是冒烟测试——它打的 `cyrusai.me` 域名 DNS 无解析，从 GitHub runner 连不上。

**根因**：`cyrusai.me` 是 2026-05-21 "security hardening" 提交（5fdde8e）里被**凭空硬编码**进冒烟脚本和 VERIFY.md 的——站点真实 canonical 域名一直是 `cyrustyj.xyz`，用户从未拥有过 cyrusai.me。某次 AI session 没核实就写了一个"看起来合理"的域名当测试目标，从那天起 CI 冒烟测试就一直打一个不存在的域名、一直红灯，没人发现。红灯常态化后就没人看了——真失败也会被淹没（狼来了效应）。

**两层教训**：
- **AI 幻觉会固化成代码债**：一次没核实的"看起来合理"的值（域名/路径/常量）一旦提交，会以"既成事实"的姿态存活很久，后续 session 还会把它当真（本案后续 session 一度误判为"域名过期"，又是二次幻觉）。
- **CI 红灯常态化 = 监控失效**。

**规则**：
1. 冒烟/监控目标用 canonical 域名，和 SEO meta（og:url / canonical link）保持一致，写之前先核实
2. CI 红灯连续 2 次就必须排查，不允许"反正一直红"常态化
3. 排查"线上挂了"类问题，先分清是**真实资产挂了**还是**配置里的值本就是错的/幻觉的**——别给一个不存在的东西编死因
